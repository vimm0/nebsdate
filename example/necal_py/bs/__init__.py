...
"""
Baishakh, Jestha, Asar, Shrawan, Bhadau, Aswin, Kartik, Mangsir, Poush, Magh, Falgun, Chaitra
...
def _days_before_year(year):
    ...
    assert MIN_YEAR <= year <= MAX_YEAR, "year must be in %s..%s" % (MIN_YEAR, MAX_YEAR)
    if year == MIN_YEAR:
        return 0
    return _DAYS_BEFORE_YEAR[year - MIN_YEAR - 1]
...
def _ord2ymd(n):
    year = MIN_YEAR + _bin_search(n, *_DAYS_BEFORE_YEAR)
    if year > MIN_YEAR:
        n -= _DAYS_BEFORE_YEAR[year - MIN_YEAR - 1]
...
class date:
    ...
    def fromordinal(cls, n):
        """Construct a date from a (MIN_YEAR, 1, 1).
    ...
    def from_datetime_date(cls, from_date):
        ...
        return cls(MIN_YEAR, 1, 1) + (from_date - _actual_datetime.date(**REFERENCE_DATE_AD))
